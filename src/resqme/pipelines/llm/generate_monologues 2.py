...
# We are in: src/resqme/pipelines/llm/generate_monologues.py
# resqme dir is two levels up from this file
RESQME_DIR = HERE.parents[2]
...
class MonologueGenerator:
    ...
    def from_csv(cls, path: str, spec: GenerationSpec) -> "MonologueGenerator":
        ...
        # If a directory was passed, assume default file inside it
        if csv_path.is_dir():
        ...
        # Robust read (handles UTF-8 w/ BOM as well)
        try:
    ...
    def _present_symptoms_for(self, disease: str) -> List[str]:
        ...
        present = row[row.astype(bool)].index.tolist()
        return present
    ...
    def run_all(self) -> List[Dict[str, Any]]:
        ...
        # If a directory was passed, write default filename inside it
        if out_path.is_dir():
        ...
        header = ["disease", "symptoms_used", "generated_call"]
